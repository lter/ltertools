...
# Read in some test data files
data_list <- ltertools::read(raw_folder = file.path("dev", "test-data"), data_format = "csv")
dplyr::glimpse(data_list[[1]])

# Make comparable list of key elements
key_list <- purrr::map(.x = names(data_list), 
                       .f = ~ dplyr::filter(key_obj, source == .x)) 
dplyr::glimpse(key_list[[1]])
...
# Invoke function
test_std <- standardize(key = key_obj, df_list = data_list, focal_file = names(df_list[1]))
