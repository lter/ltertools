...
to each raw column name. This function accepts that key and a list of
datasets that can be standardized with that key. The function
standardizes the specified dataset out of any number of datasets in the
key or list. While usable on its own, this function is intended to
streamline internal operations of
...
- focal_file:
  ...
  (character) filename corresponding to one value of "source" column of
  "key" data and to one name in "df_list".
...
- df_list:
  ...
  (list) named list of dataframe-like objects where each name is the
  filename initially containing that data

## Value

(dataframe) single standardized dataframe including all columns defined
in the "tidy_name" column of the key object
